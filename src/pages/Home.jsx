...
function Home() {
  ...
  return (
    <div className='bg-slate-200 w-full min-h-screen'>
      <Nav />
      {!input?  <div className='flex flex-wrap justify-center items-center gap-5 w-full mt-8'>
        {Categories.map((item) => (
          <div 
            key={item.name} 
            className='w-[140px] h-[150px] bg-white flex flex-col items-center justify-center gap-3 p-4 text-[18px] font-semibold text-gray-600 rounded-lg shadow-xl hover:bg-green-200 cursor-pointer transition-all duration-200 text-center'
            onClick={() => filter(item.name)}
          >
            <div className='text-4xl flex items-center justify-center'>{item.icon}</div>
      ...
      {/* Food Items Section */}
      <div className='w-full flex flex-wrap justify-center items-center pt-8 pb-8 gap-6'>
      {cate.length>1?cate.map((item) => (
        ...
        <div className='text-center text-2xl text-green-500 font-semibold pt-5'>Dish Not Found!</div>
